...
| new-api-main 独有 | 226 | 主要来自 Authz、SystemTask 其它业务 handlers、Flow、Waffo Pancake catalog/pair 体验、DataTable/RichContent/Playground 重构。SystemInstance 后端心跳、SystemInfo 实例页面、SystemTask runner/日志清理/批量渠道测试/上游模型同步/订阅维护/Midjourney 与异步任务轮询后端、SystemTask 前端面板、QuotaMath 与额度饱和审计测试已在 NexusTok 落地为原生实现差异。 |
...
| `controller` | 5 | Authz、审计、订阅 Waffo Pancake 等；SystemInfo 实例列表接口、SystemTask 只读接口、批量渠道测试 handler 和 Midjourney 轮询 handler 已按 Root 只读/系统任务边界落地。 |
...
| `controller` | 71 | 管理接口、支付、用户、渠道、日志、模型同步等业务差异；批量渠道测试、上游模型同步、订阅维护和 Midjourney 轮询已接入 SystemTask handler，继续保留为同路径实现差异。 |
...
| `controller` | 20 | 5 | 71 | 账号池控制器保留；ChannelAuthz 已先按 Root 兜底吸收渠道敏感字段 fail-closed，Codex 用量 reset、系统实例列表、SystemTask 只读接口、批量渠道测试 handler、上游模型同步 handler 和 Midjourney 轮询 handler 已补，剩余治理类控制器按 P1/P2 原生化。 |
