...
6. `preview_id` 创建成功后仍可在 TTL 内复用，后续需要改为一次性消费，避免重复提交创建多个相同渠道。
7. 刷新已有同步渠道需要稳定匹配上游 key 身份；短期可把 `platform/base_url/external_id/key_digest` 写入账号 `settings`，长期再评估是否新增三库兼容字段。
