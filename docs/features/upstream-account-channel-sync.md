...
## 2026-07-19 语义修正：渠道分组、密钥分组、倍率换算

### 三个概念必须彻底拆开

| 概念 | 实际含义 | 存储位置 | 用途 |
| --- | --- | --- | --- |
| 渠道分组 | NexusTok 下游用户可见分组 | `Channel.group` | 决定哪些 NexusTok 用户组能使用该渠道 |
| 密钥分组 | 上游平台单个 key 自己的分组 | `ChannelAccount.group` | 区分上游 key 的限制、展示和成本 |
| 倍率换算 | 上游倍率结合充值换算后的实际成本倍率 | `ChannelAccount.settings.upstream_account_sync` 元数据 + 接口返回字段 | 展示、排序和建议 priority/weight |

同步渠道的核心约束：

1. `Channel.group` 不再从上游密钥分组集合推断，管理员未填写时默认写入 `default`。
2. `ChannelAccount.group` 继续保留“密钥分组”语义，只用于展示和区分上游 key。
3. 对同步渠道，能力表和账号选择逻辑只用 `Channel.group` 判定 NexusTok 用户分组，不再把 `ChannelAccount.group` 当作下游用户分组过滤条件。
...
1. `1 RMB -> 1 USD`，上游倍率 `0.06`，倍率换算 `0.06`
2. `1 RMB -> 20 USD`，上游倍率 `0.2`，倍率换算 `0.01`
3. `1 RMB -> 6000 USD`，上游倍率 `1.5`，倍率换算 `0.00025`
4. `50 RMB -> 5000 USD`，上游倍率 `3.5`，倍率换算 `0.035`

补充规则：

1. 当“实付人民币”和“平台到账美刀额度”同时大于 0 时，启用倍率换算。
2. 任一值为空或不合法时，倍率换算直接回退为上游有效倍率。
3. 该配置只服务于同步渠道的成本展示与建议调度，不写入系统充值倍率 `TopupGroupRatio`。

### 页面与接口约束

1. 新建同步渠道和刷新同步渠道时，前端都要额外提交 `ratio_conversion`。
2. “同步密钥配置”表格的列顺序统一为：`Key / Models / Key Group / Ratio Conversion / Priority / Weight / Enabled`。
3. 渠道列表展开面板与账号池管理弹窗必须显示“密钥分组”和“倍率换算”，不能继续只显示旧的 `Group`。
4. 自动建议 priority / weight 时，排序依据改为“倍率换算”，而不是单纯的上游分组倍率。
5. 同步模式下渠道级模型是已启用同步 key 的聚合结果；管理员需要改模型时，应在逐 key 配置中修改。
