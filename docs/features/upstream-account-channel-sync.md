...
- 补充测试覆盖同优先级权重降级、跨优先级降级、自动禁用缓存更新、亲和性失效、账号池账号失败后同渠道换账号等路径。
...
- [ ] 使用真实测试平台确认接口响应、2FA/Turnstile 状态和完整 Key 读取权限。
- [x] 提交需求分析文档。

### 阶段 1：修复渠道失效降级

- [ ] 增加请求级失败渠道排除集合。
- [ ] 改造缓存/数据库选择逻辑，排除本请求已失败渠道。
- [ ] 保持 `specific_channel_id` 不降级。
- [ ] 补充后端单元测试和现有重试测试。
...
- [ ] 创建确认接口，把一个渠道和多条 `ChannelAccount` 写入同一事务。
- [ ] 支持刷新时新增、更新、禁用缺失密钥的策略。
- [ ] 写入 `balance`、`used_quota`、同步时间和同步来源元数据。
- [ ] 补充三库兼容的模型/控制器测试。

### 阶段 4：前端渠道表单

- [ ] 新增账号同步模式和平台选择。
- [ ] 新增预览结果表格，展示密钥、分组、模型倍率、建议优先级/权重。
- [ ] 支持自动应用建议和手动逐条编辑。
- [ ] 放宽账号同步模式下模型/类型的前端校验。
- [ ] 新增 i18n 翻译并运行 `bun run i18n:sync`。

### 阶段 5：真实环境验证

- [ ] 访问 `http://192.168.0.202:3003/` 登录并确认页面更新。
- [ ] 使用 MCP 浏览器验证新增渠道页面账号同步流程。
- [ ] 调用 preview/create/refresh 接口验证请求体、响应体、错误提示和权限。
...
1. new-api 与 sub2api 的实际接口字段需要以本地项目代码和真实接口验证为准；适配层不能只按页面猜测。
2. 是否允许保存平台账号密码用于后台定时刷新尚未确定。第一版默认不保存，刷新时重新输入。
3. 预览接口如何避免完整 Key 暴露给前端需要实现时细化。推荐后端短期缓存快照并返回一次性 `preview_id`，确认创建时引用该 ID。
4. 目标平台的“模型倍率”可能是系统级配置而非密钥级配置，需要确认密钥和分组的关联关系后再计算建议策略。
5. 如果平台账号没有任何密钥，创建流程应阻止落库还是允许创建禁用渠道，需要结合 UI 体验决定。
