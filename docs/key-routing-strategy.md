...
平台站点子密钥展开后，批量启用/禁用操作栏与子表共用同一个内部滚动容器，并固定在该容器顶部。横向滚动时，批量操作栏保持在当前子表可视宽度内，不随宽表内容移出；密钥操作列固定在子表右侧，并使用不透明背景和较高层级覆盖滚动中的字段内容。子表折叠或没有子密钥时，不渲染批量操作栏。外层渠道列表滚动不会把该操作栏提升为页面级固定栏。
