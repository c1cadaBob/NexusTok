...
平台站点子密钥展开后，子表根区域与渠道主表滚动容器的可视宽度对齐，不随整张渠道宽表扩展。桌面端不再创建独立的子密钥横向滚动容器，渠道主表滚动容器统一承载子密钥宽表的左右移动；子表根区域使用可视宽度和 `sticky left-0` 作为定位基准。渠道主表横向滚动时，批量启用/禁用操作栏通过同步父渠道滚动容器的位置保持在当前展开子表的可视宽度内，启用、禁用和清除选择按钮不会随宽表内容移出；子密钥操作列固定在当前可视区域右侧，并使用不透明背景和较高层级覆盖滚动中的字段内容。纵向滚动时，固定行为只作用于展开子表范围，不提升为页面级固定。子表折叠或没有子密钥时，不渲染批量操作栏。移动端继续使用移动端子密钥列表自己的滚动和顶部固定规则。
