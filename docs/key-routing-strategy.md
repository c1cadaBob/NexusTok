...
平台站点的余额和已使用额度以平台同步接口返回的 `balance`、`used_quota` 为准，同时保存到 `PlatformSiteAccount` 和父渠道；执行同步渠道或更新余额时会一起刷新这两个字段。`used_quota` 明确返回 `0` 时仍视为有效上游结果；仅当上游没有返回账号级已用额度时，才将本次同步中明确返回的子密钥已用额度求和作为回退值。管理端状态查询返回后优先展示同步结果，不使用本地请求日志或本地累计值替代。
