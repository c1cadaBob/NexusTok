...
> 文档状态：代码事实基线
> 事实基线日期：2026-09-25
> 主要代码来源：`pkg/jsplugin/`、`relay/plugin_protocol.go`、`controller/plugin_protocol.go`、`router/plugin-router.go`、`router/task-plugin-protocol-router.go`
> 关联架构文档：[`../architecture/tasks-and-plugins.md`](../architecture/tasks-and-plugins.md)、[`../architecture/relay-routing-and-conversion.md`](../architecture/relay-routing-and-conversion.md)、[`../architecture/implementation-deviations.md`](../architecture/implementation-deviations.md)
...
本文保留 v1 Manifest、路由/协议声明、字段校验、模型绑定、Host 行为和轮询状态的详细契约；架构文档负责说明这些契约如何连接到任务生命周期、Relay、计费和偏差判断。接口、Schema、类型声明或运行时校验变化时必须同步更新。

## 变更记录

| 日期 | 变更类型 | 变更前 | 变更后 | 影响范围 | 验证依据 |
| --- | --- | --- | --- | --- | --- |
| 2026-09-25 | 补充架构索引与元信息 | 已有 v1 详细契约没有统一事实基线和架构入口 | 增加事实基线、代码来源、架构分工和变更记录；保留现有英文协议细节 | Manifest、Host Protocol、路由、模型绑定和轮询 | `pkg/jsplugin/`、`relay/plugin_protocol.go`、`router/` 静态核对 |
