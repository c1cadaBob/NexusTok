...
| DEV-016 | 平台站点交互验证与阶段快照 | 计划要求遇到上游交互验证时停止重复登录、短期单次消费，并区分关键阶段失败和可选阶段告警 | NewAPI/Sub2API 识别验证页和结构化验证错误，创建五分钟三次 Challenge；后台等待期间不重复登录；八阶段状态脱敏保存，分页失败保留旧密钥，Secret/模型失败逐项隔离 | 平台站点后台同步、管理员 2FA、旧余额/密钥/能力快照、路由候选和错误排障 | 已实现 | `service/upstream_site_challenge.go`、`service/upstream_site.go`、`model/upstream_site_sync_stages.go`、`controller/upstream_channel.go` | 2026-09-25 | 持续补充真实上游只读兼容测试；不绕过上游 CAPTCHA、Turnstile、Cloudflare 或 WAF |
