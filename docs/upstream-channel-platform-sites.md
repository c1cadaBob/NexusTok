...
`GET /api/channel/:id/upstream-keys` 只返回管理员管理所需的脱敏字段：密钥 ID、父渠道 ID、外部 ID、名称、脱敏预览、模型列表、模型能力同步状态、密钥优先级、上游原始倍率、生效倍率、倍率覆盖、有效权重、自动权重、权重覆盖、状态、禁用摘要和同步时间。真实密钥、剩余额度、过期时间和最近使用时间不返回；这些字段只保留在后端模型中参与路由过滤。
