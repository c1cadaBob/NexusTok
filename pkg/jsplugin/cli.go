...
// RunCLI implements the `nexustok plugin` subcommand: linting a plugin source
// and replaying a golden fixture against it. It returns a process exit code.
func RunCLI(args []string, stdout, stderr io.Writer) int {
	if len(args) < 2 {
		fmt.Fprintln(stderr, "usage: nexustok plugin lint <plugin.js> | nexustok plugin test <plugin.js> --fixture <fixture.json>")
		return 2
	}
	...
	case "lint":
		if len(args) != 2 {
			fmt.Fprintln(stderr, "usage: nexustok plugin lint <plugin.js>")
			return 2
		}
	...
	case "test":
		if len(args) != 4 || args[2] != "--fixture" || strings.TrimSpace(args[3]) == "" {
			fmt.Fprintln(stderr, "usage: nexustok plugin test <plugin.js> --fixture <fixture.json>")
			return 2
		}
