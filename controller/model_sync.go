...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/nexustok/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/nexustok/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/nexustok/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/nexustok/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/nexustok/models.json", base), fmt.Sprintf("%s/api/nexustok/vendors.json", base)
}
