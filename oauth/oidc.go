...
func (p *OIDCProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	if err := validateConfiguredOAuthEndpointURL(settings.TokenEndpoint); err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-OIDC] ExchangeToken endpoint blocked: %s", err.Error()))
	...
	client := newConfiguredOAuthHTTPClient(5 * time.Second)
...
func (p *OIDCProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	if err := validateConfiguredOAuthEndpointURL(settings.UserInfoEndpoint); err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-OIDC] GetUserInfo endpoint blocked: %s", err.Error()))
		return nil, NewOAuthErrorWithRaw(i18n.MsgOAuthConnectFailed, map[string]any{"Provider": "OIDC"}, err.Error())
	}
