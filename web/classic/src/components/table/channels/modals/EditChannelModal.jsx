...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 50:
      return '按照如下格式输入: AccessKey|SecretKey, 如果上游是NexusTok，则直接输ApiKey';
    case 51:
...
const EditChannelModal = (props) => {
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'base_url' && value.endsWith('/v1')) {
      Modal.confirm({
        title: '警告',
        content:
          '不需要在末尾加/v1，NexusTok会自动处理，添加后可能导致请求失败，是否继续？',
        onOk: () => {
  ...
  const handleCredentialModeChange = (value) => {
    handleInputChange('credential_mode', value);
    if (value === 'multi_key') {
      setBatch(true);
      setMultiToSingle(true);
      setMultiKeyMode(inputs.multi_key_mode || 'random');
      handleInputChange('multi_key_mode', inputs.multi_key_mode || 'random');
      return;
    }
    setMultiToSingle(false);
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      const credentialMode =
        chInfo.credential_mode ||
        (chInfo.account_pool_enabled
          ? 'account_pool'
          : isMulti
            ? 'multi_key'
            : 'single_key');
  ...
  const submit = async () => {
    ...
    let credentialMode = localInputs.credential_mode || 'single_key';
    ...
    localInputs.channel_info = {
      ...
      multi_key_mode: localInputs.multi_key_mode || multiKeyMode || 'random',
    };
    ...
    if (isEdit) {
      res = await API.put(`/api/channel/`, {
        ...
        key_mode: credentialMode === 'multi_key' ? keyMode : undefined,
      });
    } else {
  ...
  const batchExtra = batchAllowed ? (
    <Space>
      {!isEdit && credentialMode === 'single_key' && (
        <Checkbox
          disabled={isEdit}
          checked={batch}
          onChange={(e) => {
              ...
              onChange={() => {
                setMultiToSingle((prev) => {
                  ...
                  setInputs((prevInputs) => {
                    ...
                    if (nextValue) {
                      newInputs.credential_mode = 'multi_key';
                      newInputs.multi_key_mode = multiKeyMode;
                    } else {
                      newInputs.credential_mode = 'single_key';
                  ...
                  <div className='p-2 space-y-3' ref={formContainerRef}>
                    ...
                    <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      {inputs.credential_mode === 'account_pool' && (
                        <>
                          <Form.Select
                            field='account_pool_mode'
                            label={t('账号池策略')}
                            optionList={[
                              { label: t('轮询'), value: 'polling' },
                              { label: t('随机'), value: 'random' },
                            ]}
                            value={inputs.account_pool_mode || 'polling'}
                            onChange={(value) =>
                              handleInputChange('account_pool_mode', value)
                            }
                            style={{ width: '100%' }}
                            extraText={t(
                              '优先级最高者优先，同优先级账号按权重轮询。',
                            )}
                          />
                          <Form.Switch
                            field='account_pool_fallback'
                            label={t('无可用账号时回退渠道密钥')}
                            checkedText={t('开')}
                            uncheckedText={t('关')}
                            checked={inputs.account_pool_fallback === true}
                            onChange={(value) =>
                              handleInputChange('account_pool_fallback', value)
                            }
                            extraText={t(
                              '仅当账号池无可用账号时，才使用渠道密钥或多 Key 列表。',
                            )}
                          />
                        </>
                      )}
                          ...
                          {inputs.type === 8 && (
                            <>
                              <Banner
                                type='warning'
                                description={t(
                                  '如果你对接的是上游One API或者NexusTok等转发项目，请使用OpenAI类型，不要使用此类型，除非你知道你在做什么。',
                                )}
                                className='!rounded-lg'
                              />
                              <div>
                            ...
                            (inputs.type !== 45 || doubaoApiEditUnlocked) && (
                              <div>
                                <Form.Input
                                  ...
                                  disabled={isIonetLocked}
                                  extraText={t(
                                    '对于官方渠道，NexusTok已经内置地址，除非是第三方代理站点或者Azure的特殊接入地址，否则不需要填写',
                                  )}
                                />
                              </div>
                            )}
