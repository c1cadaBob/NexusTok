...
- **代码注释语言**：修改或新增代码时，主体注释统一使用中文；如果触碰到同一逻辑块内原有英文注释，需要同步改为中文。接口名、函数名、类型名、字段名、HTTP header、API、SDK、OAuth、JWT、Redis、React、TypeScript、TanStack Query 等专业标识保留英文。
- **注释完整度**：注释应尽量细致完备，说明业务背景、约束来源、状态流转、兼容性原因和异常边界；避免只复述代码做了什么。涉及权限、表单校验、请求体转换、账号池、计费、缓存和跨主题兼容时，必须把关键前提写清楚。
...
- **MCP 验证**：完成前端页面、交互、后端接口或前后端联调相关改动后，必须使用 MCP 浏览器工具实际打开页面、触发关键操作，并检查页面状态、控制台错误、网络请求、请求体、响应体和权限行为。若接口没有页面入口，应在浏览器上下文中通过 MCP 发起请求并检查响应；如环境限制无法使用 MCP，必须说明原因和替代验证方式。
...
- **2026-05-21**：补充 MCP 验证要求：完成前端页面、后端接口或联调改动后必须使用 MCP 实际测试、调试和调用。
