...
export function LogsFilterToolbar<TData>(props: LogsFilterToolbarProps<TData>) {
  ...
  const mobileFilterBodyRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => {
      const firstFocusable = mobileFilterBodyRef.current?.querySelector<
  ...
  const handleMobileFiltersOpenChange = (open: boolean) => {
    // Vaul 会在打开 Drawer 时隐藏页面其它区域；先释放触发按钮焦点，
    // 避免焦点仍停留在被 aria-hidden 的页面内容中。
