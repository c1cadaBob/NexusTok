...
const logTypeRowTint: Record<number, string> = {
  ...
  [LOG_TYPE_ENUM.REFUND]: 'border-l-info bg-info/5',
}
...
function MobileLogTimeStatus({
  ...
  const logType = typeof type === 'number' ? type : LOG_TYPE_ENUM.UNKNOWN
  ...
  return (
    <div className='flex min-w-0 flex-col gap-1'>
      ...
      <div
        className={cn(
          'inline-flex w-fit items-center gap-1 text-xs leading-none font-medium',
  ...
  return (
    <div className='flex min-w-0 flex-col gap-2.5'>
      ...
      <div className='grid grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] gap-1.5'>
        <div className='bg-muted/25 flex min-w-0 flex-col gap-1 rounded-md px-2 py-1.5'>
          <div className='text-muted-foreground text-[11px] leading-none font-medium select-none'>
            {t('Time')}
          </div>
          <MobileLogTimeStatus
    ...
    <div className='bg-card overflow-hidden rounded-lg border'>
      {rows.map((row) => {
        ...
        const logType = (row.original as Record<string, unknown>).type as
          ...
          | undefined
        const tintClass =
          logType != null && logCategory === 'common'
        ...
        return (
          <div
            key={row.id}
            className={cn(
              'border-b border-l-2 border-l-transparent p-3 transition-colors last:border-b-0',
              tintClass
            )}
          >
