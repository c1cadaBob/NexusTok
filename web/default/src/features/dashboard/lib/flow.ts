...
// 这些维度可能暴露用户、密钥、部署或业务拓扑；模型名属于公开调用对象，不默认遮罩。
...
function colorAt(index: number, palette?: readonly string[]): string {
  const colors = palette && palette.length > 0 ? palette : FLOW_CHART_COLORS
...
function colorPalette(
  ...
  return Array.from(
    { length: Math.max(colorCount, FLOW_CHART_COLORS.length) },
    (_, index) => FLOW_CHART_COLORS[index % FLOW_CHART_COLORS.length]
  )
}
...
function buildFlowHighlightSets(
  ...
  // 点击边时只高亮经过该边的完整路径；否则高亮经过当前节点的完整路径。
...
export function buildFlowSankeySpec(
  ...
  return {
    ...
    // 高亮状态完全由我们写入的数据字段控制，避免 VChart Sankey 内置关联点击逻辑与全路径高亮冲突。
