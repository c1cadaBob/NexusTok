...
export async function exportPoolAccounts(
  groupId: number,
  data?: PoolAccountExportPayload
): Promise<ApiResponse<AccountPoolExportResult>> {
  const res = await api.post(
    `/api/account-pool/groups/${groupId}/accounts/export`,
    data ?? {}
  )
  return res.data
}
