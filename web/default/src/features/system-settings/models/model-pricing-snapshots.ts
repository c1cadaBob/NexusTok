...
export const buildModelSnapshots = ({
  ...
  return Array.from(modelNames).map((name) => {
    ...
    if (modeForModel === 'tiered_expr') {
      ...
      // 作为兜底值。快照必须保留这些字段，避免草稿往返时误删兼容配置。
...
export const buildModelRows = ({
  ...
  saved: ModelPricingSnapshotInput
  draft: ModelPricingSnapshotInput
}): ModelRow[] => {
  const savedRows = buildModelSnapshots(saved)
  const draftRows = buildModelSnapshots(draft)
  ...
  return Array.from(modelNames)
    .map((name) => {
      const savedRow = savedByName.get(name)
      const draftRow = draftByName.get(name)
      const displayed = draftRow ?? savedRow
      const savedSignature = getSnapshotSignature(savedRow)
      const draftSignature = getSnapshotSignature(draftRow)
      ...
      return {
        ...
        isDraftDeleted: Boolean(savedRow && !draftRow),
        isDraftNew: Boolean(!savedRow && draftRow),
      }
    })
