...
function sanitizeMarkdownHtml(html: string): string {
  const purifier = DOMPurify as typeof DOMPurify & {
    (window: Window): typeof DOMPurify
  }
  ...
  if (typeof window !== 'undefined' && typeof purifier === 'function') {
    return purifier(window).sanitize(html, sanitizeOptions)
  }
...
export function renderMarkdownForTest(markdown: string, breaks = false): string {
  const parsedHtml = markdownParser.parse(markdown, {
    ...markdownOptions,
    breaks,
  })
  const html = sanitizeMarkdownHtml(parsedHtml)
...
export function Markdown(props: MarkdownProps) {
  const html = useMemo(
    () => renderMarkdownForTest(props.children, props.breaks),
