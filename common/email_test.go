...
// newFakeSMTPServer 创建默认支持 STARTTLS 的本地 SMTP 测试服务。
func newFakeSMTPServer(t *testing.T) *fakeSMTPServer {
	return newFakeSMTPServerWithSTARTTLSAdvertisement(t, true)
}

// newFakeSMTPServerWithSTARTTLSAdvertisement 创建可控制 STARTTLS 广播能力的本地 SMTP 服务。
...
// newFakeImplicitTLSSMTPServer 创建传统 465/SMTPS 风格的隐式 TLS SMTP 服务。
...
func newTestTLSCertificate() (tls.Certificate, error) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName: "smtp.internal.test",
		},
		NotBefore:   time.Now().Add(-time.Hour),
		NotAfter:    time.Now().Add(time.Hour),
		KeyUsage:    x509.KeyUsageKeyEncipherment | x509.KeyUsageDigitalSignature,
		ExtKeyUsage: []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth},
		DNSNames:    []string{"smtp.internal.test"},
	}
...
// withSMTPSettings 保存并恢复会影响邮件发送的全局配置，防止测试之间互相污染。
...
func TestSendEmailDoesNotAutoUpgradeWhenStartTLSDisabled(t *testing.T) {
	...
	select {
	case command := <-server.startTLSCommands:
		t.Fatalf("收到非预期的 SMTP STARTTLS 命令: %s", command)
	default:
	}
...
func TestSMTPPlainAuthRejectsRemotePlaintextConnection(t *testing.T) {
	...
	case command := <-server.authCommands:
		t.Fatalf("收到非预期的 SMTP AUTH 命令: %s", command)
	default:
	}
}
...
func TestSendEmailUsesNTLMWhenServerOnlySupportsNTLM(t *testing.T) {
	...
	select {
	case command := <-server.authCommands:
		require.True(t, strings.HasPrefix(command, "AUTH NTLM "), "非预期的认证命令: %s", command)
	case <-time.After(2 * time.Second):
		t.Fatal("等待 SMTP AUTH 超时")
	}
}
