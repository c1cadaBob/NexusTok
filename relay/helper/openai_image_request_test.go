...
// TestGetAndValidOpenAIImageRequestMultipartStream 验证图片编辑 multipart 请求：
// stream 字段会被解析和校验，同时原始 body 在校验后仍可被上游转换逻辑复读。
...
// TestGetAndValidOpenAIImageRequestNBounds 保护图片数量的计费不变量：
// n 不能以超大值或负数进入后续 quota 计算，避免 int64 转换后发生溢出或反向计费。
