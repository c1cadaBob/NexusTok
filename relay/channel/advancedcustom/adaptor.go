...
// Adaptor 实现 Advanced Custom Channel。
//
// 该适配器只负责 route 解析、URL/auth 组装和 converter 分发；协议转换与响应处理复用
// OpenAI、Claude、Gemini 现有适配器，避免产生第二套兼容逻辑。
type Adaptor struct {
...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.NexusTokError) {
	...
	case dto.AdvancedCustomConverterOpenAIResponsesToOpenAIChatCompletions:
		...
		return chatToResponsesHandler(c, info, resp)
	default:
...
func (a *Adaptor) doNativeResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (any, *types.NexusTokError) {
...
func resolveUpstreamTargetURL(upstreamPath string, info *relaycommon.RelayInfo) (*url.URL, error) {
	...
	if parsedURL.User != nil {
		return nil, errors.New("advanced custom upstream path must not include user info")
	}
...
func joinBaseURLAndUpstreamPath(baseURL string, upstreamPath string) (*url.URL, error) {
	...
	if parsedBaseURL.User != nil {
		return nil, errors.New("channel base URL must not include user info when advanced custom upstream path is relative")
	}
...
func chatToResponsesHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NexusTokError) {
