...
// TestOpenaiImageStreamHandlerForwardsSSEAndUsage 覆盖图片 SSE 主路径：
// 分片会按 OpenAI Images 事件名转发，usage 会从 input/output tokens 归一化到通用计费字段，
// 并在收到上游 [DONE] 后向客户端补发结束标记。
...
// TestOpenaiImageStreamHandlerWrapsJSONResponse 覆盖上游忽略 stream=true 的兜底：
// 普通 JSON 成功响应会包装为 image_generation.completed SSE 事件。
...
// TestOpenaiImageHandlersReturnJSONError 验证非流式和流式 JSON 兜底都能识别 OpenAI error body，
// 返回结构化错误，同时不把上游错误 body 当作成功响应写给客户端。
func TestOpenaiImageHandlersReturnJSONError(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })

	body := `{"error":{"message":"content moderation failed","code":"content_moderation_failed","status":502}}`
...
// TestOpenaiImageStreamHandlerRecordsUpstreamErrorEvent 验证流中 error event 会作为软错误记录，
// 同时仍把上游 payload 转发给客户端，便于客户端展示已收到的部分结果和错误原因。
