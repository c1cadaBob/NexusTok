...
func TestCatalogIncludesNexusTokCoreResources(t *testing.T) {
	...
	assertActionExists(t, resources["channel_account"], ActionWrite)
	...
	assertActionExists(t, resources["redemption"], ActionSecretView)
	assertActionExists(t, resources["usage_log"], ActionSensitiveWrite)
	assertActionExists(t, resources["usage_data"], ActionRead)
...
func TestRolesExposeRootAndAdminBaselines(t *testing.T) {
	...
	assert.True(t, root.Grants["channel_account"][ActionWrite])
	...
	assert.True(t, admin.Grants["channel_account"][ActionWrite])
	...
	assert.True(t, admin.Grants["account_pool_auth_file"][ActionRead])
	assert.False(t, admin.Grants["account_pool_auth_file"][ActionSensitiveWrite])
	...
	assert.False(t, admin.Grants["redemption"][ActionSecretView])
	assert.True(t, admin.Grants["usage_log"][ActionRead])
	assert.False(t, admin.Grants["usage_log"][ActionSensitiveWrite])
	assert.True(t, admin.Grants["usage_data"][ActionRead])
...
func TestCapabilitiesFollowExistingSystemRoles(t *testing.T) {
	...
	assert.False(t, admin["redemption"][ActionSecretView])
	assert.True(t, admin["usage_log"][ActionRead])
	assert.False(t, admin["usage_log"][ActionSensitiveWrite])
	...
	assert.True(t, admin["channel_account"][ActionWrite])
	...
	assert.False(t, user["usage_log"][ActionRead])
	assert.False(t, user["usage_data"][ActionRead])
...
func TestCanFollowsRoleBaselinesAndFailsClosed(t *testing.T) {
	...
	assert.True(t, Can(2, common.RoleAdminUser, ChannelAccountWrite))
	...
	assert.True(t, Can(2, common.RoleAdminUser, AccountPoolAuthFileRead))
	assert.False(t, Can(2, common.RoleAdminUser, AccountPoolAuthFileSensitiveWrite))
	...
	assert.False(t, Can(2, common.RoleAdminUser, RedemptionSecretView))
	assert.True(t, Can(2, common.RoleAdminUser, UsageLogRead))
	...
	assert.False(t, Can(3, common.RoleCommonUser, UsageLogRead))
	assert.False(t, Can(3, common.RoleCommonUser, UsageDataRead))
