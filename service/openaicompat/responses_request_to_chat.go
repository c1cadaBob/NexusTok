// responses_request_to_chat.go 实现 OpenAI Responses 请求到 Chat Completions 请求的转换。
// 该转换用于让尚未原生支持 /v1/responses 的上游渠道复用现有 Chat 适配链路。
...
// ResponsesRequestToChatCompletionsRequest 将无状态 Responses 请求转换为 Chat Completions 请求。
//
// 转换只覆盖 Chat 能安全表达的子集。conversation、previous_response_id、prompt 和
// context_management 等 stateful 字段无法在 Chat 请求里保持等价语义，因此这里明确返回错误，
// 避免把有状态请求静默降级成语义不同的普通消息。
func ResponsesRequestToChatCompletionsRequest(req *dto.OpenAIResponsesRequest) (*dto.GeneralOpenAIRequest, error) {
	...
	if rawJSONPresent(req.ParallelToolCalls) && common.GetJsonType(req.ParallelToolCalls) == "boolean" {
		var parallelToolCalls bool
		if err := common.Unmarshal(req.ParallelToolCalls, &parallelToolCalls); err == nil {
			out.ParallelTooCalls = &parallelToolCalls
		}
	}
	if rawJSONPresent(req.PromptCacheKey) && common.GetJsonType(req.PromptCacheKey) == "string" {
