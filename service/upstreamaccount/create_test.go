...
func TestCreateFromPreviewCreatesChannelAndAccounts(t *testing.T) {
	...
	balanceSnapshot, ok := ReadChannelAccountBalanceSnapshot(channel.OtherSettings)
	require.True(t, ok)
	require.NotNil(t, balanceSnapshot.BalanceUSD)
	require.NotNil(t, balanceSnapshot.UsedUSD)
	require.Equal(t, float64(3.5), *balanceSnapshot.BalanceUSD)
	require.Equal(t, float64(1.2), *balanceSnapshot.UsedUSD)
	require.Greater(t, balanceSnapshot.SyncedAt, int64(0))
