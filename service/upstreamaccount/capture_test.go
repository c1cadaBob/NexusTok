...
func TestCaptureSessionCompletesNewAPIAccessTokenPayloadAndRendersScript(t *testing.T) {
	...
	require.Contains(t, helperScript, "发送登录态到 NexusTok")
	...
	require.Equal(t, "https://nexus.example.com/api/channel/upstream-account/capture-helper.user.js", status.HelperInstallURL)
	require.Equal(t, captureHelperVersion, status.HelperVersion)
	require.Contains(t, status.HandoffURL, "nexustok_capture=")
