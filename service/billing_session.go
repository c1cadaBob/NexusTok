...
// 任一步骤失败时原子回滚已完成的步骤。
func (s *BillingSession) preConsume(c *gin.Context, quota int) *types.NexusTokError {
...
func NewBillingSession(c *gin.Context, relayInfo *relaycommon.RelayInfo, preConsumedQuota int) (*BillingSession, *types.NexusTokError) {
	if relayInfo == nil {
	...
	// 钱包路径需要先检查用户额度
	tryWallet := func() (*BillingSession, *types.NexusTokError) {
	...
	trySubscription := func() (*BillingSession, *types.NexusTokError) {
	...
	default:
		...
		if apiErr != nil {
			if apiErr.GetErrorCode() == types.ErrorCodeInsufficientUserQuota {
				// 订阅套餐可声明额度耗尽后不允许钱包兜底。检查使用购买时写入
				// UserSubscription 的策略快照，避免管理员后来修改套餐影响已购买订阅。
