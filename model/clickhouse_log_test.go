...
func TestClickHouseLogTTLDays(t *testing.T) {
	original, had := os.LookupEnv("LOG_SQL_CLICKHOUSE_TTL_DAYS")
	t.Cleanup(func() {
		if had {
			require.NoError(t, os.Setenv("LOG_SQL_CLICKHOUSE_TTL_DAYS", original))
			return
		}
		require.NoError(t, os.Unsetenv("LOG_SQL_CLICKHOUSE_TTL_DAYS"))
	})

	require.NoError(t, os.Setenv("LOG_SQL_CLICKHOUSE_TTL_DAYS", "45"))
	assert.Equal(t, 45, clickHouseLogTTLDays())

	require.NoError(t, os.Setenv("LOG_SQL_CLICKHOUSE_TTL_DAYS", "-5"))
	assert.Equal(t, 0, clickHouseLogTTLDays())
}
...
func TestBuildLogLikeConditionRejectsUnsafeWildcard(t *testing.T) {
	originalLogDatabaseType := common.LogDatabaseType()
	t.Cleanup(func() {
		common.SetLogDatabaseType(originalLogDatabaseType)
	})
	...
	condition, pattern, err := buildLogLikeCondition("logs.model_name", "g%%%")
...
func TestEnsureLogRequestId(t *testing.T) {
	...
	assert.NotEmpty(t, empty.RequestId, "空 request_id 应在入库前自动补齐")

	existing := &Log{RequestId: "fixed-request-id"}
	ensureLogRequestId(existing)
	assert.Equal(t, "fixed-request-id", existing.RequestId, "已有 request_id 必须保留")
