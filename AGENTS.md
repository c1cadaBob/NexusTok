...
- 代码注释的主体内容必须使用中文；接口名、函数名、协议名、专业标识和第三方固定术语保留英文
...
- 修改已有代码时，如果触碰到同一逻辑块内的英文主体注释，应同步改为中文；接口名、函数名、字段名、HTTP header、provider、SDK、API、URL、OAuth、JWT、Redis、GORM 等专业标识保留英文
- 注释内容应细致完备，优先解释业务背景、约束来源、边界条件、兼容性考虑和“为什么这样做”，避免只复述代码表面的赋值或调用
- 对容易误用的兼容逻辑、权限逻辑、计费逻辑、请求改写逻辑、数据库迁移逻辑，注释需要明确说明输入、输出、异常路径和不变量
- 新增注释时优先写完整中文句子，必要时用列表补充步骤；不要为了简短而省略关键前提

### 规则 12：MCP 页面与接口验证 — 前后端完成后必须实际调用

完成前端页面、前端交互、后端接口或前后端联调相关改动后，必须使用 MCP 浏览器工具进行真实验证、调试和调用：

- 前端页面/交互：使用 MCP 打开对应页面，执行关键操作，检查页面状态、控制台错误和网络请求。
- 后端接口：优先通过页面触发真实请求；没有页面入口时，使用 MCP 在浏览器上下文中发起接口调用并检查响应。
- 联调功能：必须确认请求路径、请求体、响应体、错误提示和权限行为符合预期。
- 最终回复中说明已用 MCP 验证的页面/接口，以及发现并修复的问题；如受环境限制无法使用 MCP，必须明确说明原因和替代验证方式。
